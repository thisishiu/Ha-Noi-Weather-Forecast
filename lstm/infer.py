...
from train_global import GlobalLSTM  # type: ignore
...
@torch.no_grad()
def forecast_k_steps(model: GlobalLSTM, X_window: torch.Tensor, d_idx: int, k: int) -> List[torch.Tensor]:
    # X_window: [T, F] (last lookback steps), d_idx: scalar int
    model.eval()
    ...
    for _ in range(int(k)):
        ...
        y = model(inp, d)  # shape: [1, H, F] (new) or [1, F] (old)
        if y.dim() == 3:
            y_step = y[:, 0, :].squeeze(0)  # take first horizon step
        else:
            y_step = y.squeeze(0)  # backward compatibility
...
def main():
    parser = argparse.ArgumentParser(description="Iterative multi-step forecast with LSTM model")
    parser.add_argument(
        "--model-dir",
        type=str,
        default=str(Path(__file__).resolve().parent / "model"),
        help="Directory containing global_lstm.pt and global_config.json",
    )
    ...
    state = torch.load(model_dir / "global_lstm.pt", map_location="cpu")
