...
def _make_fallback_geo_table(num_districts: int, fourier_K: int) -> torch.Tensor:
    # geo feature dim = 2 (lat_norm, lon_norm) + 4 * K (sin/cos lat/lon)
...
def export_onnx(model_dir: Path, onnx_out: Optional[Path] = None) -> Path:
    ...
    model_path = model_dir / "global_lstm.pt"
    if onnx_out is None:
        onnx_out = model_dir / "global_lstm.onnx"
    ...
    # Dummy inputs (dynamic batch and sequence length supported via dynamic_axes)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Output ONNX path (default: <model-dir>/global_lstm.onnx)",
    )
