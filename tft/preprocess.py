...
def preprocess(input_path: str = "data/hanoi_weather.csv", output_root: str = "data/splits"):
    ...
    for dname, group in df.groupby("district", sort=False):
        ...
        dshow = normalize_district(dname)
        try:
            print(f"{dshow}: Train={len(group.iloc[s_train])}, Dev={len(group.iloc[s_dev])}, Test={len(group.iloc[s_test])}")
        except Exception:
            print(f"{dshow} splits: train/dev/test computed")
    ...
    print("Preprocessing + combined splitting done! (TFT)")
