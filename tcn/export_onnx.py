...
def export_onnx(model_dir: Path, onnx_out: Optional[Path] = None) -> Path:
    ...
    if onnx_out is None:
        onnx_out = model_dir / "global_tcn.onnx"
...
def main():
    parser = argparse.ArgumentParser(description="Export TCN model to ONNX")
    parser.add_argument(
        "--model-dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Output ONNX path (default: <model-dir>/global_tcn.onnx)",
    )
